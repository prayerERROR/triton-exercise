...
def benchmark_relu():
    ...
    for size in sizes:
        ...
        ms, ms_max, ms_min = triton.testing.do_bench(lambda: torch.nn.functional.relu(x), quantiles=quantiles)
        torch_perf.append((GBps(ms), GBps(ms_max), GBps(ms_min)))
        ms, ms_max, ms_min = triton.testing.do_bench(lambda: relu(x), quantiles=quantiles)
